...
import type { MultiSelectOptions } from '../types';
...
export class MultiSelectPrompt<V> extends Prompt<any[], MultiSelectOptions<V>> {
	...
	private getFilteredChoices() {
		...
		return this.options.choices
			...
			.filter((c) =>
				c.title.toLowerCase().includes(this.searchBuffer.toLowerCase()),
			);
	}
	...
	protected render(firstRender: boolean) {
		...
		const lines =
			1 +
			(choices.length === 0 ? 1 : Math.min(choices.length, this.pageSize)) +
			(this.errorMsg ? 1 : 0);
	...
	protected handleInput(char: string) {
		...
		if (this.isUp(char)) {
			...
			if (choices.length > 0) {
				this.selectedIndex =
					(this.selectedIndex - 1 + choices.length) % choices.length;
				this.render(false);
			}
			return;
		}
